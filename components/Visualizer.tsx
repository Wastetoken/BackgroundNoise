...
const fsSimSource = `
  ...
  uniform float uFeedback; // ACTS AS DRIVE (Injection Strength)
  ...
  void main(void) {
    ...
    // --- AUDIO GATE ---
    // High sensitivity to catch low volume without flashing
    float totalEnergy = uBass + uMid + uHigh;
    float isActive = smoothstep(0.001, 0.02, totalEnergy); 
    ...
    if (uMode == 0) { // VAPOR
        ...
        pattern = (v + 3.0) / 6.0;
        float band = step(0.5, fract(pattern * 5.0)); 
        genColor = mix(uColorA, uColorB, clamp(pattern, 0.0, 1.0));
        genColor += vec3(band * 0.1); 
    }
        ...
        genColor = mix(uColorA, uColorB, clamp(pattern, 0.0, 1.0));
        if (cells < 0.1) genColor += vec3(uHigh * 0.5); 
    }
    ...
    // ==========================================
        ...
        pattern = spiral * 0.5 + 0.5;
        genColor = mix(uColorA, uColorB, pattern);
    }
    else if (uMode == 7) { // TURBO
         float n = noise(p * 20.0 + t * 5.0);
         displacement = vec2(n, n) * drive * 0.5;
         pattern = n;
         genColor = mix(uColorA, uColorB, n);
    }
    else if (uMode == 8) { // SHARD
        vec2 m = abs(fract(p * 4.0 + t*0.1) - 0.5);
        float d = max(m.x, m.y);
        ...
        genColor = mix(uColorA, uColorB, pattern);
         ...
         pattern = v * 0.3 + 0.5;
         genColor = mix(uColorA, uColorB, pattern);
         genColor = hueShift(genColor, v);
    }
    else if (uMode == 10) { // ECHO
        displacement = vec2(sin(t), cos(t)) * 0.01 * (1.0 + uBass);
        pattern = distance(uv, vec2(0.5));
        genColor = mix(uColorA, uColorB, uHigh);
    }
    else if (uMode == 11) { // VORTEX
         float ang = atan(p.y, p.x);
         float d = length(p);
         float v = sin(d * 10.0 - t * 4.0);
         displacement = vec2(sin(ang + v), cos(ang + v)) * drive * 0.2;
         pattern = smoothstep(0.0, 1.0, v);
         genColor = mix(uColorA, uColorB, pattern);
    }
    else if (uMode == 12) { // CYBER
        vec2 gp = floor(p * 10.0);
        float rnd = random(gp + floor(t));
        if (rnd > 0.8) {
            displacement = vec2(rnd - 0.5, 0.0) * drive;
        }
        pattern = step(0.5, rnd);
        genColor = mix(uColorA, uColorB, pattern);
        if (rnd > 0.95) genColor = vec3(1.0);
    }
    else { // NOISE (13)
        float n = random(uv * t);
        displacement = vec2(n - 0.5) * drive * 0.5;
        pattern = n;
        genColor = mix(uColorA, uColorB, step(0.5, n));
    }
    ...
    // --- INJECTION & PAINTING ---
    // DECAY = 1.0: Infinite Persistence. No fading.
    float decay = 1.0; 
    
    // DRIVE LOGIC (0-30 Range):
    // uFeedback (0.0-1.0) * 0.3 = 0.0 - 0.3 Max Injection.
    float maxDrive = uFeedback * 0.3;
    float inputSensitivity = clamp(uBass * 2.0 + uMid * 1.5, 0.0, 1.0);
    
    // The amount of "New Paint" to mix in.
    float paintAmount = inputSensitivity * maxDrive;
    
    // --- BLENDING ---
    // Standard Mix: (Existing Canvas * 1.0) mixed with (New Paint) based on volume/drive.
    // If audio is silent, paintAmount is 0, preserving the canvas perfectly.
    vec3 result = mix(prevColor.rgb * decay, genColor, paintAmount);
    
    if (uColorShift > 0.0) {
        result = hueShift(result, uColorShift * 0.002);
    }
...
export const Visualizer: React.FC<VisualizerProps> = ({ state, audio, userImage, onExportRef }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
        ...
        const energy = a.bass + a.mid + a.high;
        if (energy > 0.05) {
        ...
        // Feedback uniform now mapped linearly, shader handles the 30% limit logic
        ...
        const cA = hexToRgb(s.colorA);
        const cB = hexToRgb(s.colorB);
        ...
        gl.bindTexture(gl.TEXTURE_2D, texturesRef.current[readIndex]);
        ...
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texturesRef.current[writeIndex]);
